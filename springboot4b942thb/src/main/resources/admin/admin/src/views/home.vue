<template>
	<div class="home-content">
		...
		<div class="statis-box">
			...
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('yueyuyingshi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-chujia3"></span>
				</div>
				<div class="right">
					<div class="num">{{yueyuyingshiCount}}</div>
					<div class="name">粤语影视总数</div>
				</div>
			</div>
			<div id="statis3" class="statis3 animate__animated" v-if="isAuth('yuejuxinxi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-qita3"></span>
				</div>
				<div class="right">
					<div class="num">{{yuejuxinxiCount}}</div>
					<div class="name">粤剧信息总数</div>
				</div>
			</div>
			<div id="statis4" class="statis4 animate__animated" v-if="isAuth('yinlexinxi','首页总数')">
				<div class="left">
		...
		<div class="type3">
			...
			<div id="yueyuyingshiChart1" class="echarts2 animate__animated" v-if="isAuth('yueyuyingshi','首页统计')"></div>
			<div id="yuejuxinxiChart1" class="echarts3 animate__animated" v-if="isAuth('yuejuxinxi','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.getyueyuyingshiCount();
		if(this.isAuth('yueyuyingshi','首页统计')){
			this.yueyuyingshiChat1();
		}
		this.getyuejuxinxiCount();
		if(this.isAuth('yuejuxinxi','首页统计')){
			this.yuejuxinxiChat1();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'yueyuyingshiChart1',css:'animate__fadeInUp'},
				{id:'yuejuxinxiChart1',css:'animate__fadeInUp'},
			]
...
// 1234 圆
		...
		getyueyuyingshiCount() {
			this.$http({
				url: `yueyuyingshi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.yueyuyingshiCount = data.data
				}
			})
		},
// 1234 饼
		yueyuyingshiChat1() {
			this.$nextTick(()=>{
				...
				var yueyuyingshiChart1 = echarts.init(document.getElementById("yueyuyingshiChart1"),'macarons');
				this.$http({
					url: "yueyuyingshi/group/yingshifenlei",
					method: "get",
				}).then(({ data }) => {
					if (data && data.code === 0) {
						let res = data.data;
						...
						for(let i=0;i<res.length;i++){
							...
							xAxis.push(res[i].yingshifenlei);
							yAxis.push(parseFloat((res[i].total)));
							pArray.push({
								value: parseFloat((res[i].total)),
								name: res[i].yingshifenlei
							})
						}
						var option = {};
						...
						// 使用刚指定的配置项和数据显示图表。
						yueyuyingshiChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							yueyuyingshiChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		getyuejuxinxiCount() {
			this.$http({
				url: `yuejuxinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.yuejuxinxiCount = data.data
				}
			})
		},
// 1234 横
		yuejuxinxiChat1() {
			this.$nextTick(()=>{
				...
				var yuejuxinxiChart1 = echarts.init(document.getElementById("yuejuxinxiChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						yuejuxinxiChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							yuejuxinxiChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
