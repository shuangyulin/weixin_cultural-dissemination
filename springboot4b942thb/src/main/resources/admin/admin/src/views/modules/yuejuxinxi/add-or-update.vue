<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="粤剧名称" prop="yuejumingcheng" >
					<el-input v-model="ruleForm.yuejumingcheng" placeholder="粤剧名称" clearable  :readonly="ro.yuejumingcheng"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="粤剧名称" prop="yuejumingcheng" >
					<el-input v-model="ruleForm.yuejumingcheng" placeholder="粤剧名称" readonly></el-input>
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'"  label="粤剧分类" prop="yuejufenlei" >
					<el-select :disabled="ro.yuejufenlei" v-model="ruleForm.yuejufenlei" placeholder="请选择粤剧分类" >
						<el-option
							v-for="(item,index) in yuejufenleiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="粤剧分类" prop="yuejufenlei" >
					<el-input v-model="ruleForm.yuejufenlei"
						placeholder="粤剧分类" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-else-if="ruleForm.shangyingriqi" label="上映日期" prop="shangyingriqi" >
					<el-input v-model="ruleForm.shangyingriqi" placeholder="上映日期" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item class="textarea" v-if="type!='info'" label="参与演员" prop="canyuyanyuan" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="参与演员"
					v-model="ruleForm.canyuyanyuan" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.canyuyanyuan" label="参与演员" prop="canyuyanyuan" >
				<span class="text">{{ruleForm.canyuyanyuan}}</span>
			</el-form-item>
			<el-form-item v-if="type!='info'"  label="粤剧介绍" prop="yuejujieshao" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.yuejujieshao" 
					class="editor" 
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.yuejujieshao" label="粤剧介绍" prop="yuejujieshao" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.yuejujieshao"></span>
			</el-form-item>
	...
	export default {
		data() {
			...
			return {
				...
				rules: {
					yuejumingcheng: [
						{ required: true, message: '粤剧名称不能为空', trigger: 'blur' },
					],
					yuejufenlei: [
						{ required: true, message: '粤剧分类不能为空', trigger: 'blur' },
					],
					fengmian: [
					],
					shipin: [
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						if(o=='yuejumingcheng'){
							this.ruleForm.yuejumingcheng = obj[o];
							this.ro.yuejumingcheng = true;
							continue;
						}
						if(o=='yuejufenlei'){
							this.ruleForm.yuejufenlei = obj[o];
							this.ro.yuejufenlei = true;
							continue;
						}
						if(o=='fengmian'){
						...
						if(o=='canyuyanyuan'){
							this.ruleForm.canyuyanyuan = obj[o];
							this.ro.canyuyanyuan = true;
							continue;
						}
						if(o=='shangyingriqi'){
						...
						if(o=='yuejujieshao'){
							this.ruleForm.yuejujieshao = obj[o];
							this.ro.yuejujieshao = true;
							continue;
						}
						if(o=='thumbsupnum'){
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.yuejujieshao = this.ruleForm.yuejujieshao.replace(reg,'../../../springboot4b942thb/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yuejuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yuejuxinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
